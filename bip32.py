...
def childKeyDerivationPrivate(parentPriavteKey, parnetChaincode, index):
  
  i_str = ser32(index)
  if index & BIP32_HARDEN:
    data = bytes.fromhex("00") + parentPriavteKey + bytes(i_str)
  else:
    data = privateToPublic(parentPriavteKey) + bytes(i_str)
  (Il, Ir) = hmacsha512(data, parnetChaincode)
  ilInt =  string_to_int(Il)
  if ilInt > CURVE_ORDER:
    return None
  privateInt = string_to_int(parentPriavteKey)
...
def main():
  ...
  masterPrivateKey = key[0]
  masterChainCode = key[1]
